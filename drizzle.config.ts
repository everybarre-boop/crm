/* ======================================================================
   ...
   · DATABASE_URL 은 Supabase Postgres 접속 문자열(전권). .env.local 에만 두고
     ...
     이 프로젝트 폴더는 OneDrive 로 동기화되므로, .env.local 을 동기화 밖에 두고
     DOTENV_PATH 환경변수로 그 경로를 가리키는 편이 안전하다(.gitignore 는 커밋만 막는다).
...
// 위치는 DOTENV_PATH 로 덮어쓸 수 있다(scripts/backup-table.mjs 와 동일 규칙).
