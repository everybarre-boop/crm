...
begin
  ...
  if not dry_run then
    ...
    on conflict (res_key) do update set
      /* 확정 상태(출석/결석/노쇼/취소)를 나중 스크랩이 **미확정 상태로 되돌리지** 않게 한다.
         ⚠️ '예약대기'도 미확정이다. 대기자와 확정자는 res_key 가 같으므로(같은 사람·같은
            수업), 대기가 승인돼 수업에 들어가면 다음날 D-1 스크랩이 이 행을
            '예약대기' → '출석' 으로 덮는다. 그게 "대기가 실제로 승인됐는지"를 다음날
            확인하는 근거다. 반대로 roster 를 재실행했을 때 '예약대기'가 이미 확정된
            '출석'을 덮어 버리면 그 근거가 사라진다. */
      "예약상태" = case
                     when excluded."예약상태" in ('예약','예약대기')
...
values
  ('milestone', '마일스톤', '🎉', true, 10, -1,
   -- 소급허용=false: 지난 마일스톤을 뒤늦게 축하하면 "이번 수업이 N번째"가 거짓말이 된다
   '{"마일스톤":[10,30,50,100,150,200,300,500],"소급허용":false,"소급한도":10}'::jsonb,
   -- {{누적횟수}} = 내일 수업을 뺀 실제 누적(마일스톤−1). 2026-08-27 정정 — 자세한 배경은
   -- sql/2026-08_crm_rules_v3.sql 과 shared/crm-rules.mjs 의 DEFAULT_RULES 주석.
   '*{{마일스톤}}회차!* (지금까지 {{누적횟수}}회 · 이번 수업이 {{마일스톤}}회차)',
   '{{이름}}님, 이번 수업이 {{마일스톤}}번째예요! 꾸준히 나오시는 게 정말 대단해요 👏'),
