...
export const DEFAULT_RULES = [
  {
    ...
    /* 소급허용=false — 정확히 마일스톤 회차일 때만 보낸다.
       true 로 두면 "이미 지난 마일스톤"을 뒤늦게 축하하는데, 문구가 "이번 수업이 N번째"라
       **사실과 다른 말**이 된다(실측: 누적 51회 회원에게 "50번째 수업").
    ...
    /* 🔥 {{누적횟수}} 는 **아직 오지 않은 내일 수업을 뺀** 실제 누적이다(마일스톤−1).
       예전에는 이 자리에 예정회차를 넣어 "10회차! (누적 10회)" 로 렌더했는데, 발송 시점의
       members 누적은 9 라 강사가 스튜디오메이트와 대조하면 **항상 1 이 부족했다**
       (2026-08-27 확인: 전 지점 발송 21건 전부). 두 수를 나란히 보여 대조가 되게 한다. */
    템플릿: '*{{마일스톤}}회차!* (지금까지 {{누적횟수}}회 · 이번 수업이 {{마일스톤}}회차)',
    예시멘트: '{{이름}}님, 이번 수업이 {{마일스톤}}번째예요! 꾸준히 나오시는 게 정말 대단해요 👏',
  },
  {
  ...
  {
    /* 1회권을 반복 구매하는 드롭인 단골. first-paid 는 최대누적 3 으로 막혀 있어서
       이분들에겐 아무 멘트도 안 나갔다(실측: '체험 후 1회권' 등록 18건 · 누적 17회).
       "신규"가 아니라 단골이므로, 축하가 아니라 **정기권 제안**이 맞는 자리다.
       정렬순서 35 — 진짜 신규(30)가 있으면 그쪽이 이기고, 체험(40)보다는 우선한다. */
    id: 'dropin-return', 라벨: '1회권 재방문', 이모지: '🔁', 활성: true, 슬랙발송: true,
    정렬순서: 35, 재발송억제일수: 30,
    파라미터: { 키워드: ['1회권'], 최소누적: 4 },
    템플릿: '1회권으로 다시 방문 · {{수강권명}} (누적 {{누적횟수}}회)',
    예시멘트: '{{이름}}님, 또 오셨네요 반가워요! 정기 수강권으로 등록하시면 더 편하게 오실 수 있어요 😊',
  },
  {
...
/* ⚠️ DB(crm_rules)에 아직 없는 **규칙 자체**를 코드 기본값으로 채워 내보낸다.
   슬랙 단계는 fetchRules() 결과만 보고 "슬랙발송" 여부와 라벨을 정하므로, 새 규칙을
   코드에만 추가하면 멘트는 만들어지는데 **슬랙에 안 나가고 라벨도 없다**(조용히 꺼진다).
   파라미터 병합(ruleMap)과 같은 이유·같은 원칙이다. seed SQL 과 짝으로 쓴다. */
export function mergeRules(rules) {
  return [...ruleMap(rules).values()];
}
...
/* ----------------------------------------------------------------------
   이 예약이 **실제로 쓰는** members 행 고르기.
   🔥 예전엔 수강권명만 맞춘 뒤 pickTicketRow(= 가장 최근 시작일)로 골랐다. 같은 이름의
      수강권을 여러 번 등록한 회원에게서 이게 틀린다 — 실측(2026-08-22 옥수 이수정):
      '체험 후 1회권' 등록건이 18개인데 코드는 08-10 시작(09-08 만료)을 골랐고, 그 수업이
      실제로 쓴 건 07-27 시작(08-25 만료 · 화면 표기 "4일 남음")이었다. 만료임박을 통째로 놓친다.
   ✅ reservations 행이 그 수업이 쓰는 수강권의 **시작일·종료일**을 이미 갖고 있다
      (실측 433건 중 시작일·종료일 100% 채워짐). 추측하지 말고 그걸로 정확히 맞춘다.
      스크랩이 시작일을 못 읽었을 때만 옛 방식(가장 최근 등록건)으로 폴백한다.
   ---------------------------------------------------------------------- */
export function pickTicketForReservation(memberRows, resv) {
  const same = (memberRows || []).filter((m) => m.수강권명 === resv.수강권명);
  if (!same.length) return null;
  const 시작 = ymdNum(resv.수강권시작일);
  if (시작 !== null) {
    const exact = same.filter((m) => ymdNum(m.수강권시작일) === 시작);
...
export function buildCrm({
  ...
  for (const r of roster) {
    ...
    const ticket = pickTicketForReservation(mem, r);
    ...
    // ── ③-2 1회권 재방문 (드롭인 단골) ────────────────────────────
    /* first-paid 가 최대누적으로 막는 사람들 중 **1회권을 또 산 회원**만 따로 잡는다.
       최소누적(4) = first-paid 최대누적(3) + 1 로 맞춰 둬서 두 규칙이 겹치지 않는다. */
    if (!체험수업 && ticket) {
      const p = R.get('dropin-return').파라미터 || {};
      const 키워드 = p.키워드 || ['1회권'];
      const 최소누적 = toInt(p.최소누적) || 4;
      const 해당 = 키워드.some((kw) => kw && String(r.수강권명 ?? '').includes(kw));
      if (해당 && usedCount(ticket) === 0 && 누적횟수 >= 최소누적) {
        push('dropin-return', '', {}, {
          누적횟수, 수강권시작일: ticket.수강권시작일 ?? '', 수강권명: r.수강권명,
        });
      }
    }
    ...
    if (ticket) {
      ...
      /* 🔥 최소전체횟수 — 1회권·체험권 제외. 전체=1·잔여=1 은 비율이 **항상 1.0** 이라
         비율 조건을 무조건 통과한다. 그래서 체험권 회원에게 "아직 1회 남으셨어요.
         추가 등록하시면 남은 횟수는 그대로 이월돼요!" 가 나갔다
         (2026-08-22 청담 이송은·김예지 실측 — expiring 12건 중 2건).
         1회권은 "다 쓰면 끝"이 정상이라 이월을 권할 대상이 아니다. 게다가 정렬순서상
         만료임박(20)이 체험(40)을 이겨서 **올바른 체험 멘트 대신** 이게 나갔다. */
      const 최소전체횟수 = toInt(p.최소전체횟수) || 2;
