...
### 주간 교차검증 루틴 — 두 기록을 계속 맞춰 둔다

교차검증은 **두 기록이 살아 있을 때만** 작동한다. `members`(엑셀)가 낡거나 `reservations`
(스크랩)에 구멍이 나면, 검증은 "모순"이 아니라 그냥 **보류**만 쌓는다. 그래서 주 1회 아래를 돈다.

**① 회원 엑셀 재업로드 (주 1회 · 관리자 직접)**

예약사이트에서 전 지점 회원 파일을 내려받아 관리자 화면 → `데이터 업로드` 에 올린다.
스튜디오메이트의 수강권명 표기 변경·이름 표식 변화로 생긴 출석 미매칭이 여기서 대부분 해소된다.

- ⚠️ **올리기 전에 이전 파일과 헤더를 대조할 것.** 키 컬럼 구성이 달라지면 `dedup_key` 가 통째로
  바뀌어 "전량 중복"이 된다(CLAUDE.md — 실제로 두 번 겪었다).
- 업로드 후 점검: `npm run db:sql sql/2026-08_verify_upload.sql`
  — 행수가 아니라 **"최근 날짜가 실제로 들어왔는가"** 로 본다.

**② 누적 횟수 점검 (읽기 전용)**

```bash
npm run db:sql sql/2026-08_verify_used_count.sql
```
...
| 표 | 무엇을 보나 | 위험 신호 |
|---|---|---|
| ① 부풀림 규모 | 행 합 vs 등록건 합 | `부풀려진_인원` 이 **갑자기 늘면** 중복 행이 새로 생긴 것(표식·결제 분할) |
| ② 마일스톤 경계 인원 | 곧 회차를 밟을 사람의 발송/보류 판정 | `보류` 가 대다수면 스크랩이나 엑셀 한쪽이 낡았다 |
| ③ 이미 나간 마일스톤 | 발송 회차 ≤ 현재 누적 + 1 인가 | `⚠️ 아직 도달하지 않은 회차` 가 뜨면 오발송이다 → `sql/2026-08_fix_milestone_overcount.sql` |

**③ 예약 스냅샷에 구멍이 없는지**

야간 실행이 빠진 날이 있으면 그날은 관측이 통째로 비어, 그 기간에 다닌 회원의 출석이
교차검증에서 누락된다(= 보류가 늘어난다). 빈 날짜는 백필로 메운다:

```bash
# 먼저 건수만 (저장 안 함)
DRY_RUN=true BACKFILL_FROM=2026-08-15 BACKFILL_TO=2026-08-18 node automation/backfill.mjs
# 실제 저장
DRY_RUN=false BACKFILL_FROM=2026-08-15 BACKFILL_TO=2026-08-18 node automation/backfill.mjs
```

백필은 `reservations` 만 채우고 `apply_attendance` 는 부르지 않는다 — 수업 상세의 "12회 남음"은
그 날짜의 값이 아니라 **회원의 현재 잔여횟수**라, 과거 날짜로 members 를 갱신하면 같은 값을
수백 번 덮어쓸 뿐이다. 즉 백필은 **교차검증의 근거(출석 기록)만 두껍게 한다** — 누적 자체는
건드리지 않는다. 관측 시작일이 앞당겨지면 "전 이력 관측" 판정 대상이 늘어 검증이 더 촘촘해진다.

**④ 매일 아침 30초 — 보류가 있었나**

`CRM 실행` 화면의 근거 칩(`누적 N회` · `출석기록 N회` · `중복행 N건 접음`)과
`자동화 로그` 화면의 경고를 본다. `마일스톤 N건 보류` 가 보이면 그날은 회차를 확정하지 못한
회원이 있었다는 뜻이다 — 원인은 대개 ①(엑셀이 낡음) 또는 ③(스냅샷 구멍)이다.
...
| `first-paid` 신규 등록 첫 수업 | 체험 이력 있음 + 유료 수강권 사용 0회 + **전 지점 누적 ≤ 3** | ✅ | 평생 1회 |
| `dropin-return` 1회권 재방문 | 수강권명에 '1회권' + 그 등록건 사용 0회 + **전 지점 누적 ≥ 4** → 정기권 제안 | ✅ | 30일 |
| `expiring` 만료 임박 | 종료일 7일 이내 **AND** 잔여 ≥ 전체의 30% **AND 전체 ≥ 2회**(체험권 제외) **AND 이미 재등록한 회원 제외**(§5-1) → "이월 가능" 안내 | ✅ | 7일 |
| `dormant-14` 14일 미방문 | 마지막 출석 후 14일 이상 + 잔여>0 + 기간 남음 | ❌ 대시보드 명단만 | — |

**1인 1건 우선순위**(= `정렬순서`, 작을수록 우선):
마일스톤(10) > 만료임박(20) > 신규(30) > 1회권 재방문(35) > 체험(40) > 14일 미방문(50).

### 5-1. 만료 임박 — 이미 재등록한 회원은 뺀다 (2026-09-09)

"추가 등록하시면 남은 횟수는 그대로 이월돼요!" 는 **다음 수강권을 이미 끊은 회원에게는
틀린 말**이다. 등록을 두 번 권하는 꼴이라 현장에서 신뢰를 깎는다.
→ 파라미터 `재등록제외: true`(기본 켜짐). 판정은 `shared/crm-rules.mjs` 의 `hasRenewalTicket`.

**🔥 판정 기준은 "종료일 이후에 시작한 수강권"이 아니다.** 실측(2026-09-09 재업로드 후 ·
members 18,897행):

| 같은 사람의 연속한 등록건 9,995쌍 | 건수 | 비율 |
|---|---|---|
| 이전 종료일 **이후**에 시작 | 3,506 | 35% |
| 같은 날 시작 | 131 | 1% |
| **이전 종료일보다 먼저 시작(겹침)** | **6,358** | **63.6%** |

중앙값은 **−14일**이다 — 결제한 날부터 새 수강권이 열리기 때문에, 재등록은 옛 권이
끝나기 전에 시작하는 게 오히려 정상이다. 그래서 "종료일 이후 시작"으로 걸면 그날의
만료 임박 후보 31명 중 4명만 걸려 **실제 재등록자 6명 중 2명(33%)을 놓친다.**

✅ 대신 **그 만료일 이후까지 유효한(= 더 늦게 끝나는) 다른 등록건**이 있으면 뺀다(6명 전원).
2026-09-09 예약 187건으로 재현하면 만료임박 대상 4건이 **전부** 이 조건에 걸렸다
(`만료임박_재등록제외 4 · expiring 0`) — 조건이 없었으면 4건이 그대로 오발송이었다.

세부 규칙:
- **등록건 단위로 접고 비교한다**(`dedupeTicketRows`). 같은 등록건의 중복 행(이름 표식·
  결제 분할)을 안 접으면 자기 자신이 "다음 수강권"이 되어 전원이 제외된다.
- **잔여 0 인 권은 다음 수강권이 아니다**(이미 끝난 권). 다만 잔여가 **비어 있으면**
  유효한 것으로 본다 — 모를 때는 보내지 않는 쪽이 안전하다.
- 뺀 건수는 `stats.만료임박_재등록제외` 로 남는다. 조용히 0건이 되면 규칙이 꺼진 것과
  구별되지 않기 때문이다.
- ⚠️ **근거는 `members` 다 — 회원 엑셀 업로드가 밀리면 그 사이 재등록은 안 보인다.**
  같은 날 실측으로 증명됐다: 업로드가 8일 밀린 상태(최신 `수강권시작일` 09-01)에서는
  후보 34명 중 **2명**만 걸렸는데, 재업로드 직후 같은 조건이 31명 중 **6명**을 걸렀다.
  **업로드가 밀리면 이 조건은 조용히 절반만 일한다.**
  주 1회 재업로드 루틴(§4-1 "주간 교차검증 루틴")이 이 조건의 전제다.
- 반영 SQL: [../sql/2026-09_crm_rules_v4.sql](../sql/2026-09_crm_rules_v4.sql).
  끄려면 관리자 화면에서 `재등록제외` 를 `false` 로 둔다.

### 🔥 2026-08-22 실발송에서 드러난 오분류 2건 (수정 완료)

**① 만료 임박이 1회권·체험권에 발동했다.** 판정이 `잔여/전체 ≥ 0.3` 뿐이라
**전체=1·잔여=1 이면 비율이 항상 1.0** 이라 무조건 통과한다. 그래서 체험권 회원에게
"아직 1회 남으셨어요. 추가 등록하시면 남은 횟수는 그대로 이월돼요!" 가 나갔다
(청담 이송은·김예지 — expiring 12건 중 2건). 1회권은 "다 쓰면 끝"이 정상이라 이월을
권할 대상이 아니다. 게다가 **정렬순서상 만료임박(20)이 체험(40)을 이겨서 올바른 체험 멘트까지
밀어냈다** — 틀린 멘트가 맞는 멘트를 밀어내는 구조라 피해가 두 배였다.
→ `최소전체횟수: 2` 신설 + 코드에서 체험권 자체도 제외.

**② 같은 수강권을 여러 번 등록한 회원의 수강권을 잘못 골랐다.** 옛 코드는 수강권명만
맞춘 뒤 "가장 최근 시작일"을 집었다. 옥수 이수정님은 '체험 후 1회권' 등록건이 **18개**인데,
코드는 08-10 시작(09-08 만료)을 골랐고 그 수업이 실제로 쓴 건 07-27 시작(08-25 만료,
화면 표기 "4일 남음")이었다.
→ **`reservations` 행이 그 수업이 쓰는 수강권의 시작일·종료일을 이미 갖고 있다**
(실측 433건 중 100% 채워짐). 추측하지 말고 `수강권명 + 수강권시작일` 정확 일치로 고른다
(`pickTicketForReservation`). 스크랩이 시작일을 못 읽었을 때만 옛 방식으로 폴백한다.

⚠️ **새 규칙을 코드에만 추가하면 슬랙에 안 나간다.** 발송 단계는 `fetchRules()`(DB 행)의
`슬랙발송`·`라벨`만 보기 때문이다. `mergeRules()` 로 코드 기본값을 병합해 넘기고,
관리자 화면에서 편집하려면 seed SQL(`sql/2026-08_crm_rules_v2.sql`)도 함께 돌린다.
파라미터 병합(`ruleMap`)과 같은 원칙이다.
