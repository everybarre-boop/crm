...
- 🔥 **사람 단위 누적 사용횟수는 행을 더해서 내지 말 것 — `personUsedCount(rows)` 를 쓴다.**
  ...
  **그 행들이 같은 잔여횟수를 각자 들고 있다.** `Σ (전체−잔여)` 는 한 번 나온 수업을 2~3번 센다.
  ...
  - ⛔️ **멘트에 "누적 N회"를 쓸 때 그 N 은 members 에 실제로 있는 값이어야 한다.**
    마일스톤은 **내일 수업으로 N 회를 채우는 회원에게 전날 밤** 나가므로(`예정회차 = 누적 + 1`),
    발송 시점의 누적은 언제나 `마일스톤 − 1` 이다. 옛 템플릿은 그 자리에 예정회차를 덮어써
    `*10회차!* (누적 10회)` 로 단언했고, 강사가 스튜디오메이트와 대조하면 **항상 정확히 1 이
    부족**했다(2026-08-27 확인 — 발송 21건 전부, 10·30·50·100 · 6개 지점 예외 없음).
    지금은 두 수를 나란히 적는다: `(지금까지 9회 · 이번 수업이 10회차)`.
    **발송 시점은 그대로다** — "이번 수업이 10번째"는 강사가 수업에서 말할 때 사실이다.
    정정 SQL 은 [sql/2026-08_crm_rules_v3.sql](sql/2026-08_crm_rules_v3.sql)
    (문구 교체 + 옛 문구로 나간 발송 이력 삭제 — 평생 1회 억제가 진짜 축하를 막지 않도록).
...
매일 21:07 KST GitHub Actions 실행(21:43 · 22:26 · 23:11 예비) → 어제 출석 반영 → 내일 예약자 명단 →
...
- 🔥 **회차·마일스톤의 근거는 `전체횟수 − 잔여횟수` 가 아니라 실제 출석 기록이다**
  ...
  - 🔥 **캘린더에는 수업이 아닌 블록도 있다 — `.event-item.etc`(기타 일정: 대관·외부행사).**
    ...
    원인은 단 하나였다 — 이틀짜리 대관 블록 '한국바레협회29기'. `classItem` 을 `:not(.etc)` 로 걸렀다.
    기타 일정에는 예약자 목록이 없으므로(상세 확인: 0명) 빼도 잃는 데이터가 없다.
  - **수업 블록 하나가 안 열려도 그 사이트를 통째로 버리지 않는다** — 클릭은 2회 재시도하고,
    끝내 안 열리면 **그 수업만** 건너뛰며 나머지를 살린다(사이트 전체 실패는 로그인부터 1회 더).
    ⚠️ 건너뛴 수업은 `누락` 으로 fail() 되어 운영 채널에 건수가 간다 — 조용히 넘기지 않는다.
...
- 🔥 **"재등록했는가"를 "옛 수강권 종료일 **이후에 시작**한 권이 있는가"로 판정하지 말 것.**
  결제한 날부터 새 권이 열리기 때문에, 같은 사람의 연속 등록건 9,995쌍 중 **6,358쌍(63.6%)이
  이전 종료일보다 먼저 시작**한다(중앙값 −14일 · 2026-09-09 실측 members 18,897행).
  그 기준으로는 그날 재등록자 6명 중 4명만 잡힌다. 올바른 기준은 **그 만료일 이후까지
  유효한(= 더 늦게 끝나는) 다른 등록건**이다 — [shared/crm-rules.mjs](shared/crm-rules.mjs)의
  `hasRenewalTicket`(만료 임박 멘트에서 이미 재등록한 회원을 뺀다 · 파라미터 `재등록제외`).
  ⚠️ 근거가 `members` 라 **회원 엑셀 업로드가 밀리면 그 사이 재등록은 안 보인다.** 실측:
  업로드가 8일 밀렸을 때 후보 34명 중 2명 → 재업로드 직후 31명 중 **6명**이 걸렸다.
  **밀리면 이 조건이 조용히 절반만 일한다** — 주 1회 재업로드가 전제다.
