...
- **`dedup_key` 불변식(중요) = 중복 판정 기준:** upsert 충돌 판단용 고유 키.
  ...
  - **`members` 키(`KEY_COLS`) = `이름 · 연락처 · 수강권명 · 등록일 · 전체횟수`.**
    이 5개가 모두 같으면 "같은 데이터"로 보고 재업로드 시 덮어쓴다(1건만 유지).
  - `sales` 키(`SALES_KEY_COLS`)는 결제 1건을 식별하는 필드 집합(이름·연락처·생년월일·
    수강권명·결제구분/금액/일시/방법/할부).
...
- **`used_count`(사용횟수) 필터 컬럼:** `members.used_count` = `전체횟수 − 잔여횟수`인 STORED
  생성 컬럼(숫자 외 문자는 제거 후 계산). 회원 관리/대시보드의 "사용횟수 범위" 필터가
  이 컬럼을 `.gte/.lte`로 서버에서 거른다. 생성 컬럼이라 **업로드 upsert 에 넣지 말 것**.
